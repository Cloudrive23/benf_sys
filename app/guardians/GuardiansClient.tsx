...
export default function GuardiansClient() {
  ...
  async function load() {
    ...
    if (data.success) {
      ...
      toast.error(data.message || "تعذر تحميل بيانات المعيلين");
    }
  }

  async function loadNextCode() {
    const res = await fetch("/api/guardians/next-code", {
      cache: "no-store",
    });
  ...
  function openEdit(item: Guardian) {
    setForm({
      ...
      notes: item.notes || "",
      is_mother: item.is_mother ?? false,
  ...
  async function save(e: React.FormEvent) {
    ...
    await saveEntityWithPolicies({
      ...
      errorMessage: "تعذر حفظ بيانات المعيل",
      onSuccess: async () => {
        setOpen(false);
        setForm(emptyForm);
        await load();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">إدارة المعيلين</h1>

          <p className="text-sm mt-1" style={{ color: "var(--app-muted)" }}>
            إدارة بيانات المعيلين وربطهم بالفروع والقيم المرجعية
          </p>
        </div>
        ...
        <DynamicEntityTable
          ...
          renderValue={(row, field) => {
            ...
            if (field.field_name === "is_alive") {
              return <Badge>{row.is_alive ? "على قيد الحياة" : "متوفى"}</Badge>;
            }

            if (field.field_name === "is_mother") {
              return row.is_mother ? "نعم" : "لا";
            }
